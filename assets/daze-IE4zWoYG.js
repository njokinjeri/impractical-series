...
`,w=[b,P,S,A],I=["Dither Liquid","Volumetric Horizon","Viscous Dispersion","Generative Patterns"],m=["THICK CHEVRON WEAVE","RETRO PARALLEL RIBBONS","CONCENTRIC ARC OVERLAPS","HALFTONE DOT MATRIX","CITRUS RADIAL SLICES","ALIVE SCATTER SPRINKLES","VORONOI JIGSAW TILING"];document.addEventListener("DOMContentLoaded",()=>{const l=new T("webgl-canvas"),t=new E;let e=null,i=3,a=1,o=2,r=1,s=.15;const d=performance.now(),f=u(n=>{e&&e.dispose(),e=new x(l.gl,L,w[n]),i=n;const c=n===3;p.updateShaderNav(n,I[n],m[a],c)},"loadShaderEnvironment");new R(n=>{o=n});const p=new M(n=>{r=n},n=>{s=n},()=>{document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen()});t.onInteract=()=>{i===3&&(a=(a+1)%m.length,p.updatePatternDisplay(a,m[a]))},document.querySelectorAll("#shader-buttons button").forEach(n=>{n.addEventListener("click",()=>{const c=parseInt(n.getAttribute("data-index")||"0",10);f(c)})}),window.addEventListener("keydown",n=>{n.key>="1"&&n.key<="4"&&f(parseInt(n.key,10)-1)}),f(3);const v=u(()=>{if(e&&e.program){e.use(),l.bindQuad(0);const n=l.gl,c=e.uniforms,g=(performance.now()-d)*.001*r;n.uniform2f(c.uResolution,l.canvas.width,l.canvas.height),n.uniform1f(c.uTime,g),n.uniform2f(c.uTouch,t.touchPos[0],t.touchPos[1]),n.uniform2f(c.uTouchDelta,t.touchDelta[0],t.touchDelta[1]),n.uniform1f(c.uTouchActive,t.touchActive),n.uniform1f(c.uGrain,s),n.uniform1i(c.uPalette,o),n.uniform1i(c.uPatternIndex,a),t.update(),l.draw()}requestAnimationFrame(v)},"render");requestAnimationFrame(v)});
