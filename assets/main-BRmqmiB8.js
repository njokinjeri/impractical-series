var j=Object.defineProperty;var h=(t,e)=>j(t,"name",{value:e,configurable:!0});import"./modulepreload-polyfill-BdX5DvLD.js";import{s as ee,a as te,m as ne}from"./footer-mount-BRz2agx6.js";import{g as re,a as oe,A as J,p as P}from"./projects-0thjQsIe.js";const ae="#33D17A",ie="#3FA0E0",W="#E8A33D",ce=1200,le=2800,G=.022,se=160,X=7.5,z=.96,Y=.06,de=9;function he(t){const e=t.getContext("2d"),i=window.devicePixelRatio||1;let o=0,l=0,f=0;function v(){const r=t.parentElement.getBoundingClientRect();o=r.width,l=r.height,t.width=o*i,t.height=l*i,e.setTransform(i,0,0,i,0,0)}h(v,"resize"),v();const c=[...Array.from({length:3},(r,a)=>({type:"circle",x:Math.random()*o,y:Math.random()*l*.6,vx:0,vy:0,r:10,color:ae,phase:a*2.1})),...Array.from({length:3},(r,a)=>({type:"square",x:Math.random()*o,y:Math.random()*l*.6,vx:0,vy:0,r:9,color:ie,phase:a*1.7+1})),{type:"triangle",x:o/2,y:l/2,vx:0,vy:0,r:12,color:W,phase:0}],u=c.findIndex(r=>r.type==="triangle"),p=[];let m=-1,L=0,_=0,T=Date.now(),O=Date.now(),I=0;function Z(r){switch(e.save(),e.strokeStyle=r.color,e.lineWidth=1.3,e.shadowColor=r.color,e.shadowBlur=de,r.type){case"circle":e.beginPath(),e.arc(r.x,r.y,r.r,0,Math.PI*2),e.stroke();break;case"square":{e.beginPath();const a=r.r*.95;e.roundRect?e.roundRect(r.x-a,r.y-a,a*2,a*2,4):e.rect(r.x-a,r.y-a,a*2,a*2),e.stroke();break}case"triangle":e.beginPath(),e.moveTo(r.x,r.y-r.r),e.lineTo(r.x+r.r*.95,r.y+r.r*.8),e.lineTo(r.x-r.r*.95,r.y+r.r*.8),e.closePath(),e.stroke();break}e.restore()}h(Z,"drawShape");function B(){I++;const r=Date.now(),a=r-T>ce;if(c.forEach((n,d)=>{d!==m&&(n.x+=Math.sin(I*.02+n.phase)*Y,n.y+=Math.cos(I*.025+n.phase)*Y,a&&d!==u&&c.forEach((s,y)=>{if(d===y||s.type!==n.type)return;const x=s.x-n.x,E=s.y-n.y,g=Math.hypot(x,E)||1;g>n.r+s.r+4&&g<se&&(n.vx+=x/g*G,n.vy+=E/g*G)}),n.vx*=z,n.vy*=z,n.x+=n.vx,n.y+=n.vy,n.x<n.r&&(n.x=n.r,n.vx*=-.5),n.x>o-n.r&&(n.x=o-n.r,n.vx*=-.5),n.y<n.r&&(n.y=n.r,n.vy*=-.5),n.y>l-n.r&&(n.y=l-n.r,n.vy*=-.5))}),a&&r-O>le){O=r;const n=c.filter((g,k)=>k!==u),d=n[Math.floor(Math.random()*n.length)],s=c[u],y=d.x-s.x,x=d.y-s.y,E=Math.hypot(y,x)||1;s.vx=y/E*X,s.vy=x/E*X}for(let n=0;n<c.length;n++)for(let d=n+1;d<c.length;d++){const s=c[n],y=c[d],x=s.x-y.x,E=s.y-y.y,g=Math.hypot(x,E),k=s.r+y.r;if(g<k&&g>0){const C=x/g,F=E/g,H=(k-g)/2,U=s.type==="triangle"||y.type==="triangle",N=U?H+3:H,A=U?1.3:.15;n!==m&&(s.x+=C*N,s.vx+=C*A,s.vy+=F*A*.3),d!==m&&(y.x-=C*N,y.vx-=C*A,y.vy-=F*A*.3)}}p.push({x:c[u].x,y:c[u].y}),p.length>6&&p.shift(),e.clearRect(0,0,o,l),p.forEach((n,d)=>{const s=d/p.length*.35;e.save(),e.globalAlpha=s,e.fillStyle=W,e.beginPath(),e.arc(n.x,n.y,3,0,Math.PI*2),e.fill(),e.restore()}),c.forEach(Z),f=requestAnimationFrame(B)}h(B,"tick");function $(r){const a=t.getBoundingClientRect();if("touches"in r&&r.touches.length>0)return{x:r.touches[0].clientX-a.left,y:r.touches[0].clientY-a.top};if("changedTouches"in r&&r.changedTouches.length>0)return{x:r.changedTouches[0].clientX-a.left,y:r.changedTouches[0].clientY-a.top};const n=r;return{x:n.clientX-a.left,y:n.clientY-a.top}}h($,"getCanvasPos");function b(r){"touches"in r&&r.preventDefault();const{x:a,y:n}=$(r);c.forEach((d,s)=>{const y="touches"in r?d.r+16:d.r+8;Math.hypot(a-d.x,n-d.y)<y&&(m=s,L=a-d.x,_=n-d.y,T=Date.now())})}h(b,"onStart");function S(r){if(m<0)return;"touches"in r&&r.preventDefault();const{x:a,y:n}=$(r);c[m].x=a-L,c[m].y=n-_,c[m].vx=0,c[m].vy=0,T=Date.now()}h(S,"onMove");function w(){m=-1}h(w,"onEnd");const q=new ResizeObserver(v);return q.observe(t.parentElement),t.addEventListener("mousedown",b),window.addEventListener("mousemove",S),window.addEventListener("mouseup",w),t.addEventListener("touchstart",b,{passive:!1}),window.addEventListener("touchmove",S,{passive:!1}),window.addEventListener("touchend",w),window.addEventListener("touchcancel",w),f=requestAnimationFrame(B),()=>{cancelAnimationFrame(f),q.disconnect(),t.removeEventListener("mousedown",b),window.removeEventListener("mousemove",S),window.removeEventListener("mouseup",w),t.removeEventListener("touchstart",b),window.removeEventListener("touchmove",S),window.removeEventListener("touchend",w),window.removeEventListener("touchcancel",w)}}h(he,"mountHeroSandbox");function ue(t,e,i){switch(t.save(),t.strokeStyle=i,t.lineWidth=1.2,t.globalAlpha=.45,e.type){case"circle":t.beginPath(),t.arc(e.x,e.y,e.r,0,Math.PI*2),t.stroke();break;case"square":{t.beginPath();const o=e.r*.9;t.rect(e.x-o,e.y-o,o*2,o*2),t.stroke();break}case"triangle":t.beginPath(),t.moveTo(e.x,e.y-e.r),t.lineTo(e.x+e.r*.9,e.y+e.r*.7),t.lineTo(e.x-e.r*.9,e.y+e.r*.7),t.closePath(),t.stroke();break}t.restore()}h(ue,"drawThumbShape");function V(t,e){const i=window.devicePixelRatio||1,o=t.parentElement,l=o.clientWidth,f=o.clientHeight;if(!l||!f)return;t.width=l*i,t.height=f*i,t.style.width=l+"px",t.style.height=f+"px";const v=t.getContext("2d"),c=J[e.accent],u=ee(te(e.slug));v.setTransform(i,0,0,i,0,0);const p=["circle","square","triangle"],m=2+Math.floor(u()*2),L=Array.from({length:m},()=>({type:p[Math.floor(u()*3)],x:l*(.2+u()*.6),y:f*(.2+u()*.6),r:6+u()*8}));v.clearRect(0,0,l,f),L.forEach(_=>ue(v,_,c))}h(V,"drawCardThumb");function fe(t){const e=re(t.slug),i=oe(t.slug),o=String(t.index).padStart(2,"0");return`
    <article
      class="card"
      role="listitem"
      data-slug="${t.slug}"
