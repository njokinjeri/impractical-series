...
function slatBg(
  ...
  const dim =
    0.96 + 0.04 * (1 - Math.abs(index / Math.max(total - 1, 1) - 0.5) * 2);
...
function applyCast(
  ...
  const [lit, cx, bx] = t > 0 ? ['0%', '14%', '6%'] : ['100%', '86%', '94%'];
  ...
  els.castTop.style.background = `
        ...
        ${rc(PALETTE.periwinkle, abs * 0.8)} 0%, ${rc(PALETTE.periwinkle, abs * 0.32)} 28%, transparent 55%),
        radial-gradient(ellipse 36% 100% at ${periAt},
        ${rc(PALETTE.periwinkle, abs * 0.5)} 0%, transparent 46%)
        `;
  ...
  els.castLow.style.background = `
        ...
        ${rc(PALETTE.navy, abs * 0.94)} 0%, ${rc(PALETTE.crimson, abs * 0.7)} 24%,
...
export function render(
  ...
  slats.forEach((s, i) => {
    ...
    s.style.boxShadow =
      orientation === 'vertical'
        ? `${(dir * +off).toFixed(1)}px 0 ${spread}px ${rc(PALETTE.orange, abs * 0.4)},${(-dir * +off * 0.45).toFixed(1)}px 0 ${(+spread * 0.65).toFixed(1)}px ${rc(PALETTE.navy, abs * 0.38)},0 0 ${(+spread * 0.35).toFixed(1)}px ${rc(PALETTE.periwinkle, abs * 0.24)}`
        : `0 ${(dir * +off).toFixed(1)}px ${spread}px ${rc(PALETTE.orange, abs * 0.4)},0 ${(-dir * +off * 0.45).toFixed(1)}px ${(+spread * 0.65).toFixed(1)}px ${rc(PALETTE.navy, abs * 0.38)},0 0 ${(+spread * 0.35).toFixed(1)}px ${rc(PALETTE.periwinkle, abs * 0.24)}`;
  });
