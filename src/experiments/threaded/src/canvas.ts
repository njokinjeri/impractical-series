...
import { particles, stampedNibbles, clearParticles, spawnParticle, stampNibble, 
	fadeStampedNibbles, } from './particles'
import { addLangsToLog, resetTransBar, addTransByte, fadeTransBar, setEngineState, } from './ui'
...
export const state: CanvasState = {
	W: 0, H: 0, CX: 0, CY: 0,
  	device: 'desktop', threadCount: 4, vertical: false,
}
...
export function initCanvas(): void {
	measure()
  	window.addEventListener('resize', () => { measure(); if (!isPaused) resetRound() })
}
...
function measure(): void {
	...
	state.threadCount = state.device === 'mobile' ? 2 : state.device === 'tablet' ? 3 : 4
}

// Geometry: horizontal layout -- desktop

function hSpread(): number { return (state.H - BAR_H) * 0.62 }
function hSrcX():   number { return 16 }
function hDstX():   number { return state.W - 16 }
...
function vSpread(): number { return state.W * 0.72 }
function vSrcY():   number { return 70 }
function vDstY():   number { return state.H - BAR_H - 70 }
...
function nextBatch(): Language[] {
  	...
  	for (let i = 0; i < state.threadCount; i++) {
    	...
    	batch.push({ ...src, _charIndex: 0, _spawnTimer: i * 3, _spawnDelay: 4 + i * 0.4 })
  	}
  return batch
}
...
export function pauseEngine():  void { isPaused = true;  setEngineState('paused') }
export function resumeEngine(): void { isPaused = false; setEngineState('running') }
