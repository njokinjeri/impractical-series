...
export function tickSwarm(delta: number, time: number): void {
  ...
  for (let i = 0; i < activeCount; i++) {
    ...
    const swayZ =
      Math.cos(time * (u.swaySpeed * 0.8) + u.seed) * u.swayRadius * delta;

    l.position.x += swayX + windVectorX * delta * 3.0;
    l.position.z += swayZ + windVectorZ * delta * 3.0;

    l.rotation.z =
      -windVectorX * 0.2 + Math.sin(time * u.swaySpeed + u.seed) * 0.05;
    l.rotation.x =
      windVectorZ * 0.2 + Math.cos(time * u.swaySpeed + u.seed) * 0.05;
