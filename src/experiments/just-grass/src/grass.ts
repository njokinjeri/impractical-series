...
export class GrassField {
  ...
  setWind(strength: number) {
    ...
    this.windTarget = this.overgrown
      ? Math.max(strength, 1.8)
      : strength
  }
  ...
  private populate() {
    ...
    for (let col = 0; col < totalCols; col++) {
      ...
      for (let b = 0; b < bladesPerCol; b++) {
        ...
        this.blades.push(new GrassBlade({
          x: bx, baseY,
  ...
  private bindEvents() {
    ...
    c.addEventListener('mouseleave', () => { this.pointer.active = false })
    c.addEventListener('touchend',   () => { this.pointer.active = false })
  }
  ...
  private interactBlades() {
    ...
    for (const blade of this.blades) {
      ...
      const bx = blade.x, by = blade.baseY
      ...
      for (const f of [0, 0.3, 0.6, 1.0]) {
        const mt = 1 - f
        const sx = mt*mt*mt*bx + 3*mt*mt*f*cp1x + 3*mt*f*f*cp2x + f*f*f*tx
        const sy = mt*mt*mt*by + 3*mt*mt*f*cp1y + 3*mt*f*f*cp2y + f*f*f*ty
  ...
  private triggerOvergrowth() {
    ...
    for (const blade of this.blades) {
      ...
      blade.growthTarget = Math.min(maxH, blade.height * (1.3 + Math.random() * 0.5))
    }
  }
