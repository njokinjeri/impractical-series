...
const Library = (() => {
  ...
  const updateUI = () => {
    ...
    if (status)
      status.innerText = `SECRETS FOUND: ${state.found.length} / ${KEYS.length}`;
  };
  ...
  const stackBooks = () => {
    document
      ...
      .forEach((b) => b.classList.remove('tilted'));
  };

  const refreshVault = () => {
    const shelfUnit = document.getElementById(
      'shelf-unit'
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    .getElementById('stack-books')
    ?.addEventListener('click', Library.stackBooks);
  document
    .getElementById('reset-books')
    ?.addEventListener('click', Library.refreshVault);
});
